...
const Form = () => {
    ...
    const countTasksInColumn = id => {
        const numberTasksInColumn =
            tasks && tasks.filter(item => Number(item.idColumn) === Number(id)).length;
    ...
    return (
        <section className="section-form">
            ...
            <form className="form" onSubmit={e => handleForm(e)}>
                {fields.map(({ name, value, desc, placeholder }) => (
                ...
                <label htmlFor="idColumn" className="form__label">
                    Faza realizacji
                </label>
                <select
                    className="form__input"
                    ...
                    onChange={e => dispatch({ type: 'change', element: e.target })}
                >
                    {optionList.map(({ name, value, title }) => (
                        <option
                            name={name}
                            value={value}
                            key={value}
            ...
            {err.length > 0 && isActive && (
                <section className="errors">
                    ...
                    <ul>
                        {err.map(({ text, id }) => (
                            <li className="errors__item" key={id}>
                                {text}
                            </li>
                        ))}
                    </ul>{' '}
